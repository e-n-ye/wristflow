...
核验日期：2026-09-23。适用范围：个人 WristFlow 原型，黄山派尚未到货。有限范围的 grill-with-docs 已完成，用户选择 Community 图形编辑器预览/导出 C，CI 编译提交的生成代码。本机真实预览、官方 GUI 导出、主机测试与固定 SDK 编译已通过，详见 [视觉演示与操作步骤](UI-DEMO.md)。
...
上表 SDK 相对路径均以 `vendor/SiFli-SDK/external/lvgl_v9/` 为根。修复采用显式几何/样式、view 属性中的组件参数、实际字体资源和 y=423 分页点；表盘、固定 slot/布局模板和 picker 页面 [渲染证据](evidence/2026-09-22-ui/overview.png) 已留存。
...
首个 XML 链路实验按最多 2 小时的建议执行。实际完成表盘、三种固定 slot、四种布局模板、三种 picker 页面和控制中心；真实预览、官方 GUI 导出、主机 2/2 测试和固定 SDK 本机编译通过。验收仍分层：
...
3. 通过后将 slot 配置、同尺寸替换、长按意图和持久化接到 `watch_core`，继续按 XML 语义、实际渲染、目标编译和主机行为测试分层验证。静态演示不要求先实现业务状态机。
