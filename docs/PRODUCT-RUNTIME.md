...
更新：2026-09-26。本文保留首个 USB 产品框架及 2026-09-25 真机证据。应用入口 `feabc1d` 已上板；此后的 [反馈修复](PRODUCT-UI-FEEDBACK.md) 与 [组件编辑保存](COMPONENT-EDITOR.md) 已通过主机测试及两目标编译，尚未烧录。完整需求见 [产品应用与组件编辑](PRODUCT-UI-SCOPE.md)；设置扩展及亮灭屏仍待接入，不能沿用下列旧硬件结果宣称新源码通过。
