...
当前 `ui/xml/` 尚未接入固件构建，固定 SDK 内置 LVGL 9.4.0；免费 Community 编辑器与受许可限制的官方 CLI 不能混为一谈。上述生成 C 部署目标保留，具体预览和导出路线待确认与验证。核心零堆分配约束不扩展为整个 LVGL UI 零堆分配承诺。
...
1. **等板期**：
   - 本次先完成有限范围的工具链/契约复核，按 [LVGL Pro 说明](LVGL-PRO.md) 的有界实验验证一个 390×450 表盘，再扩展演示页面；语法校验不代替真实渲染。
   - `watch_core` 核心状态机与手势事件路由器仍为后续纯 C 原型及主机测试任务；本次视觉演示不要求提前实现完整核心。
2. **硬件到货第一阶段（到货验收）**：
