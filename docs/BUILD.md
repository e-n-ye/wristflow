...
本页各节结果仅适用于注明的日期与源码版本。最新已编译产品固件为 [应用入口增量](PRODUCT-APP-ENTRY.md)，本机主机 8/8、Product/UI Demo 构建通过，尚未烧录；已上板证据见 [产品运行记录](PRODUCT-RUNTIME.md)。当前硬件状态与下一项产品目标见 [STATUS](STATUS.md)。下方早期“没有硬件”“尚未烧录”不是当前全部项目的状态。
